...
public class AI_window extends JFrame {
	...
	private backend.level difficulty;
	 ...
	 * Create the frame.
	 */
	public AI_window(String server_ip,String server_port,String in_difficulty){
		...
		//setting up AI
		if(in_difficulty=="easy") {difficulty=backend.level.easy;}
		if(in_difficulty=="medium") {difficulty=backend.level.medium;}
		if(in_difficulty=="hard") {difficulty=backend.level.hard;}
			...
			try {
				while(true) {
					...
					if(response.startsWith("INFO"))
					{
						...
						{
							ba=new backend(pit_num,seed_num,original_time,false,backend.mode.two_player,difficulty);
						}
						else
